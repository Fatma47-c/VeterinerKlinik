...
namespace VeterinerKlinik.Controllers
{
    public class RandevuController : Controller
    {
        ...
        {
            var hastalıklar = new Dictionary<string, List<(string Hastalik, string Tedavi, decimal Ucret)>>
{
    { "Kedi",   new() {
    ...
    { "Van Gölü Canavarı", new() {      
        ...
        ("Kimlik Bunalımı", "Ayna terapisi", 9999) } }
            ...
            if (randevu.HayvanTuru != null && hastalıklar.ContainsKey(randevu.HayvanTuru))
            {
                var liste = hastalıklar[randevu.HayvanTuru];
